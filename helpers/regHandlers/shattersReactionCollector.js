	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		if (reaction.emoji.id == '679186891463196673'){ // key
			...
			if (!afkObj['postafk']){
                ...
                // MOVE OUT
                var reactedPortal = [];
                var reactIDS = await afkMsg.reactions.cache.get('679186863264628736').users.fetch();
	...
	reactionCollector.on('remove', async (reaction, user) => {
		...
		if (reaction.emoji.id == '679186891463196673'){ // key
			if (afkObj['key'] == user.id){
				...
				cli.channels.cache.get(cfg.shattersServer.rlcommands).send(`${user} unreacted from being the main ${cli.emojis.cache.find(e => e.id == "679186891463196673")}.`);
