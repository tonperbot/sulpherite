async function lock(cli, cfg, data){
    ...
    await cli.channels.cache.get(cfg.shattersServer.vc[args[1]].id).createOverwrite(cfg.shattersServer.raiderRole, { 'VIEW_CHANNEL': true, 'CONNECT': false, 'SPEAK': false });
    await cli.channels.cache.get(cfg.shattersServer.vc[args[1]].id).setName(cfg.shattersServer.vc[args[1]].name);
    await data.channel.send(`Locked channel ${cfg.shattersServer.vc[args[1]].name.toLowerCase()}`);
}
