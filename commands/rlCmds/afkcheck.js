...
async function afkcheck(cli, cfg, data){
	...
	if (args[2].toLowerCase() == "s" || args[2].toLowerCase() == "shatters"){
		...
		const afkMsg = await afkChannel.send(`@here Shatters (${cli.emojis.cache.get('760021427545440276')}) started by ${cli.users.cache.get(afkObj['host'])} in ${cfg.shattersServer.vc[args[1]].name}!`);
