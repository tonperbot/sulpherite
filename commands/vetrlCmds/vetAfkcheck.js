...
async function afkcheck(cli, cfg, data){
	...
	if (!args[1] || isNaN(args[1]) || (!isNaN(args[1]) && (args[1] > cfg.shattersServer.vetRaidingChannelsAmount || args[1] < 1))) return data.channel.send(`Please use a valid channel (1-${cfg.shattersServer.vetRaidingChannelsAmount})!`);
	...
	if (args[2].toLowerCase() == "s" || args[2].toLowerCase() == "shatters"){
		...
		console.log(`Started a new afk check in ${cfg.shattersServer.vetVc[args[1]].name} by ${cli.guilds.cache.get(cfg.shattersServer.id).members.cache.get(afkObj['host']).displayName}`);
		...
		// edit undefined object variables
		const afkMsg = await afkChannel.send(`@here Void Entity (${cli.emojis.cache.get('686217948880568384')}) started by ${cli.users.cache.get(afkObj['host'])} in ${cfg.shattersServer.vetVc[args[1]].name}!`);
		const cpMsg = await cpChannel.send(`AFK Check control panel for ${cfg.shattersServer.vetVc[args[1]].name}`);
		afkObj['afkcheck'] = afkMsg.id;
		afkObj['controlpanel'] = cpMsg.id;
		try {
		...
		// edit undefined object variables
		const afkMsg = await afkChannel.send(`@here Cultist Hideout (${cli.emojis.cache.get('686217948918185994')}) started by ${cli.users.cache.get(afkObj['host'])} in ${cfg.shattersServer.vetVc[args[1]].name}!`);
		const cpMsg = await cpChannel.send(`AFK Check control panel for ${cfg.shattersServer.vetVc[args[1]].name}`);
		afkObj['afkcheck'] = afkMsg.id;
		afkObj['controlpanel'] = cpMsg.id;
		...
		// edit undefined object variables
		const afkMsg = await afkChannel.send(`@here Fungal Cavern (${cli.emojis.cache.get('686223695827828774')}) started by ${cli.users.cache.get(afkObj['host'])} in ${cfg.shattersServer.vetVc[args[1]].name}!`);
		const cpMsg = await cpChannel.send(`AFK Check control panel for ${cfg.shattersServer.vetVc[args[1]].name}`);
		afkObj['afkcheck'] = afkMsg.id;
		afkObj['controlpanel'] = cpMsg.id;
